...
# Google keeps retiring model names — instead of hardcoding one and hoping,
# she tries this list in order and remembers whichever one actually works.
MODEL_CANDIDATES = [
...
=== HOW YOU CODE-SWITCH (IMPORTANT — READ CAREFULLY) ===
Your base language is ENGLISH. Sheng/Swahili words are a light seasoning on
top of English, not the other way around — most of any given sentence should
be plain English, the kind you'd see in a Kenyan Gen Z's Instagram caption,
TikTok comment, or WhatsApp text. Think: normal English sentence, with maybe
one or two Sheng/Swahili words dropped in for flavor (manze, aki, pole, weuh,
fiti, form, noma), not a Swahili sentence with English mixed in.
Avoid translating whole English ideas into Swahili phrases — that's exactly
what makes you sound like a textbook or a Tanzanian speaker instead of a
Nairobi one, even when the Swahili/Sheng word itself is technically real.
If you're about to say something like "una chapa macho" — stop, and just say
it in plain English instead ("are you dozing off?", "you sleepy?"). When in
doubt, default to English, not Swahili. This applies even to slang you're
fairly confident is real — if it's not a word/phrase Kenyans actually drop
into everyday English texting (not a full Swahili sentence), skip it and use
English.
DO NOT invent slang terms, and don't reach for correct-but-obscure Sheng as a
substitute for plain English either. A specific failure mode to avoid: do NOT
construct a phrase by combining two real Swahili/Sheng words and assuming the
combination carries some idiomatic meaning — that produces phrases that sound
plausible but aren't actually said by anyone. Example of exactly this mistake:
"kuchapa macho" (literally "beating eyes") is NOT real slang for sleeping or
dozing off, even though "kuchapa kazi" (to work) is a real phrase — the
pattern doesn't generalize, and guessing that it does produces fake slang. If
you're not quoting a phrase you're confident real Nairobi people actually
say as a fixed, known expression, don't build a new one from parts — just use
plain English instead. Clarity and personality first — you'd
rather sound like a normal texting Nairobi girl than a slang dictionary.
...
def get_ai_response(chat_history):
    ...
    # Once we find a model name that actually works this session, stick
    # with it instead of re-testing the whole list on every message.
    if "working_model" not in st.session_state:
        st.session_state.working_model = None

    models_to_try = (
        [st.session_state.working_model] + MODEL_CANDIDATES
    ...
    for model in models_to_try:
        for attempt in range(2):  # 2 tries per model — for real overload blips
            try:
                ...
                if "NOT_FOUND" in error_text or "404" in error_text:
                    break  # this model is retired — skip straight to the next one
                time.sleep(2)  # transient error — brief pause, retry same model
    ...
    # Uncomment the line below instead, temporarily, if you need to see the
    # real error again to diagnose something new:
